...
pub static mut GLOB_LIST_ARRAY_PTR: *mut *mut *mut u8 = std::ptr::null_mut();
...
pub fn init() {
    ...
    unsafe {
        GLOB_LIST_ARRAY_PTR = *((scanner.find(signature!("8b 15 ?? ?? ?? ?? 8b 14 82 85 d2 74 a7 83 42 10 01 8b 83 f4 00 00 00")).unwrap()).add(2) as *mut *mut *mut *mut u8);
...
pub fn get_list(list: Value) -> *mut List {
    unsafe {
        ...
        let datum_ptr = *(array_ptr.add(list.data.list.0 as usize));
        return datum_ptr as *mut List;
    }
}
